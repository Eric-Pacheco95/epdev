...
- Auto-invoke `/synthesize-signals` when unprocessed (signals/+failures/+absorbed/, not in signal_lineage.jsonl) >= 35 OR last synthesis >72h with any unprocessed; don't auto-invoke `/update-steering-rules`.
- **Skill friction check**: for each skill used, flag friction (missing steps, confusing params, unnecessary confirmations, unclear output) as `skill-improvement` signal with name, issue, proposed fix.
- **Skill gap check**: scan for reusable ad-hoc tasks; score Recurrence (weekly+), Repeatability (scriptable), Value; surface High-on-2+/3 as `## Skill Gap Candidates` (name, description, recurrence). No auto-invoke `/create-pattern`.
