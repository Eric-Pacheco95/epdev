...
- [ ] **Learning Pipeline Phase 2: Wisdom Tier** — DEFERRED from Phase 1 retention fix (PRD: `memory/work/jarvis/PRD_learning_pipeline_retention.md`). 30-day evaluation gate: by 2026-05-10, assess whether existing tiers (CLAUDE.md steering rules + TELOS_LEARNED + auto-memory) absorb domain/system knowledge adequately, or if a dedicated wisdom tier is needed. Scope: wisdom doc format, promotion logic from synthesis themes (established -> proven -> wisdom), human approval gate, separation from TELOS_LEARNED (Eric-knowledge) vs system/domain knowledge. Trigger: 30 days of synthesis accumulating under Phase 1 warehouse model.
