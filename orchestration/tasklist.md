...
- [ ] **Karpathy Phases 2–4 execution (5 sessions)** — Prereq for `large-extract-pattern.md` → `/create-pattern` skill promotion (2nd confirming use on a different corpus). Queue: `memory/work/karpathy/queue.json` — 10 videos pending (2 processed 2026-04-19). Method: Batch Mode per pattern lines 50-105 (pop 2 IDs, parallel subagents with template, main agent writes knowledge files). Session plan: S1 `EWvNQjAaOHw+zduSFxRajkE`; S2 `l8pRSuU81PU+kCc8FmEb1nY` (heavy, may split); S3 `VMj-3S1tku0+PaCmpygFfXo`; S4 `TCH_1BHY58I+P6sfmUTpUmc`; S5 `q8SA3rM6ckI+t3YJ5hKiMQ0`. **Suggested mid-course check after S3** (6 videos processed) to decide ai-infra vs `foundation-ml/` sub-domain before S4-5. After S5 → Phase 3 (evaluate) + Phase 4 (decide) across all 12. | Gate: none (manual kickoff per session); promotion session gated on completion.
