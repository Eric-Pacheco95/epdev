...
- [ ] **Karpathy Phases 2–4 execution (5 sessions)** — Prereq for `large-extract-pattern.md` → `/create-pattern` skill promotion (2nd confirming use on a different corpus). Queue: `memory/work/karpathy/queue.json` — 4 videos pending (8 processed through S3). Method: Batch Mode per pattern lines 50-105 (pop 2 IDs, parallel subagents with template, main agent writes knowledge files). Session plan: S1 `EWvNQjAaOHw+zduSFxRajkE` ✓; S2 `l8pRSuU81PU+kCc8FmEb1nY` ✓; S3 `VMj-3S1tku0+PaCmpygFfXo` ✓ (2026-04-21); S4 `TCH_1BHY58I+P6sfmUTpUmc`; S5 `q8SA3rM6ckI+t3YJ5hKiMQ0`. After S5 → Phase 3 (evaluate) + Phase 4 (decide) across all 12. **S4 opening decision gate (from S3 wrap-up 2026-04-21):** Both S3 subagents independently proposed `new_domain:foundation-ml/`. Micrograd + makemore-bigram sit architecturally *below* the existing LLM-systems notes (math-of-training vs systems-on-top-of-models). Before popping S4 IDs, read S3 files + new Part 2/3 (MLP + BatchNorm) after extraction — if clustering holds, **create `memory/knowledge/foundation-ml/_context.md` and move 6 foundation notes** (micrograd, makemore-bigram, makemore Parts 2–5) there; leave the 4 LLM-systems notes (llm-intro, deep-dive-llms, how-i-use-llms, gpt-tokenizer, reproduce-gpt2-124m, build-gpt-from-scratch) in `ai-infra/` — wait, two of those (reproduce-gpt2, build-gpt) are transformer-specific and sit *between* the layers; decide their home during the S4 opening review. If Parts 2–3 drift toward transformer-specific territory, keep everything in `ai-infra/` and close the foundation-ml/ question. Update `queue.json.written_files` paths if the move happens. | Gate: none (manual kickoff per session); promotion session gated on completion.
- [ ] **Pattern promotion session: `large-extract-pattern.md` → `/create-pattern` skill** — Follows Karpathy completion. Bundles: `/architecture-review` (3-agent), `/second-opinion` on `--flag` vs standalone, promotion + retirement triggers documented in pattern MD, script-vs-SKILL split (move `yt-dlp` + VTT cleanup + keyword/overlap scan to `tools/scripts/corpus_extractor.py`; keep only Phase 3 eval + Phase 4 decision in SKILL.md). Evidence base: TheCodingGopher (2026-04-19 + 2026-04-20) + Karpathy (pending). | Gate: Karpathy Phases 2–4 complete.
