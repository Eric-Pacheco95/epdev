...
> Extracted from `orchestration/autonomous-rules.md` (moved to `orchestration/steering/` to
> consolidate all sub-steering files under one namespace).
>
...
- `heartbeat_config.json` — context_files entry (Phase B: update path from `orchestration/autonomous-rules.md`)
