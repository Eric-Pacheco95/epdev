...
- **Scaffolded-but-disabled routines (`enabled: false` in `routines.json`) create invisible pipeline gaps — the system looks complete in code but produces zero artifacts.** Why: prediction-backtest pipeline was scaffolded with all wiring in place but `enabled: false`; the gap was invisible until manual triage of the routines registry. How to apply: any routine flipped to `enabled: false` must add a `disabled_reason` field and a re-evaluation date; vitals/watchdog must distinguish "scheduled but disabled" from "scheduled and silent" — disabled routines surface in the operations dashboard with their `disabled_reason`, not as healthy.
