...
- **Anthropic SDK or platform-level features (Managed Agents, Memory tool, hosted Skills runtime, multi-agent orchestration GA, Claude-managed RAG) carry a default-adopt-no prior — Jarvis is Claude-Code-hosted, file-canonical, single-operator while these features assume multi-tenant or SDK-direct callers.** When /research targets such a feature, spend the brief on the narrow integration-fit subquestion ("where in autonomous loops, if anywhere, would this be load-bearing?") — do not re-derive the verdict via full landscape coverage. Why: 3 independent /research evaluations (Memory tool 2026-04-27, Managed Agents 2026-04-08, earlier Paperclip/n8n) reached the same absorb-not-adopt verdict via the same reasoning chain; the prior is now stable enough to encode.
