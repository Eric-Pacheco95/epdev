...
| Producer/signal/alert design (idle-is-success, dedup, alert cadence, attribution) | `orchestration/steering/producer-signal-design.md` |
| Process/lock/worktree safety (mutex ownership, spawn-site fixes, worktree integrity) | `orchestration/steering/process-lock-safety.md` |
...
- **On any hard-gate failure (ISC validator STOP at ≥3 hard fails, security STOP, advisor blocker, `HARD HALT:` line from a skill), surface the exact failure output to Eric before any inline fix attempt — do not absorb, reframe, or rationalize past the gate.** Eric is the authoritative decision-maker on restructure vs. fix-inline vs. archive. Why: `isc-governance.md` scopes the same rule to PRD sessions only; this applies universally. Evidence: `scope-inflation-and-stop-gate-bypass.md` failure (sev 7).
- **Apply ceremony tier per `orchestration/steering/ceremony-tier.md`.** Tier is derived from the 4 Task Typing axes via `tools/scripts/ceremony_tier.py`; the per-phase × tier action table specifies advisor()/arch-review triggers, PRD draft cadence, and HARD HALT points (tier ≥3). Trust-boundary, MCP-class, and cross-cutting changes route through the table's THINK/T3-4 cell (`P-ARCH-REVIEW`). Full trigger detail and MCP-class taxonomy guard remain in `orchestration/steering/model-effort-routing.md`; failed-fix escalation in `orchestration/steering/incident-triage.md#I1`.
...
- **Done-marker + git hygiene.** After any session that modifies code, run `git status` before declaring done — if uncommitted changes exist, commit before closing; "task complete" / "bot restarted" / "session handed off" are not proxies for git state. When Eric says "stop prompting me about X" or "already done," update STATUS.md (Current Blockers, Queued Next Session) + the active handoff file inline in the same session — not next session. At session start, run `git log --oneline -- <path>` for each non-trivial Queued Next Session item before treating it as pending. Why: 4 sessions of crypto-bot changes went unversioned while session context described the bot as running; pagefile / transform_content.py SDK migration / handoff Effort D each persisted as pending despite git evidence of completion (signals `2026-04-27_done-marker-memory-gap.md`, `2026-04-27_handoff-stale-data-pattern.md`).
- **Claude Max cap fallback (CCR) and tool-incompatibility verification:** see `orchestration/steering/platform-specific.md` Claude Max Cap Fallback section.
...
**50 active skills available.** Skills are auto-discovered at session start. Run `/jarvis-help` for the full registry.
