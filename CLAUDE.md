...
- **When a user references any handoff file immediately before /compact or a new-session transition, verify the file path exists. If missing, block and offer to draft it — do not assume the file was written.** Why: a missing handoff PRD strands a fresh session with no durable context; incidental catch is not systematic detection.
