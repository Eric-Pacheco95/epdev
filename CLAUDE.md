...
- **After any silent-failure incident, the relaunch sequence must include at least one new detection mechanism targeting the failure CLASS, not just a fix for the specific bug.** Why: 2026-04-08 crypto-bot recovery — building NEW restart gates targeting the failure class (not just the specific bug) and relaunching let the still-active failure mode validate the gate in production within 60 seconds (real Telegram 401s caught). The failing environment is a free integration test for detection improvements, but only if you build the detector before you fix the cause. How to apply: pairs with the anti-criterion ISC rule — if a silent failure happened, the forbidden state had no verifier; adding one is mandatory before restart.
...
- **Cross-platform clock-resolution assumptions must be verified on Windows before relying on `time.time()` for any uniqueness invariant.** Why: 2026-04-08 — `backlog_append` corrupted 3 task ids in one session and 5 historical records from 2026-04-06 because the comment "microsecond-resolution to avoid collisions" was true on Linux but false on Windows (~15ms tick); successive calls within that window all hashed to the same bucket. How to apply: any code deriving identity, ordering, or deduplication from `time.time()` must either (a) use `time.time_ns()` plus a process-local counter (`last_id = max(time.time_ns(), last_id + 1)`), or (b) carry a self-test that runs N rapid successive calls and asserts uniqueness on Windows specifically.
...
- **Before editing any file outside the current working repo (`C:/Users/ericp/Github/epdev`), run `git status --short` in the target repo. If the tree is non-empty OR HEAD is not on main/master, do NOT apply Edit — propose the fix as a backlog row, a worktree-off-main patch, or a handoff note.** Why: 2026-04-08 — edited `crypto-bot/dashboard/app.py:1351` while a concurrent session had 10 uncommitted files on `fix/paper-exit-price-resolver`; my edit would have been bundled into their PR if they committed blindly. How to apply: the session-start "N Claude sessions detected" warning is a pre-edit gate for cross-repo work, not ambient noise — fires in current repo but the risk lives in every repo those sessions could touch.
- **When entering a non-epdev repo, verify before assuming: (1) `git remote show origin | grep 'HEAD branch'` for default branch, (2) check README/entry points for canonical launcher, (3) `git check-ignore <path>` before staging.** Why: 2026-04-08 — four distinct same-day frictions (crypto-bot uses `master` not `main`; canonical launcher is `launch_paper_validation.py` not `start_bot.bat`; runtime state files churn working tree; backlog rows had wrong repo paths) each cost 2-5 tool calls. How to apply: first action when entering or returning to a non-epdev repo after any gap.
