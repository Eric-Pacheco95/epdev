...
- **Three patterns worth absorbing into existing dispatcher** (cheaper than migration): (1) typed event taxonomy for dispatcher emit + jarvis-app dashboard; (2) declarative `permissions.yaml` layered over `security/validators/`; (3) `agent + environment = session` decomposition for Tier-2 task records — bundle with data-relocation backlog.
