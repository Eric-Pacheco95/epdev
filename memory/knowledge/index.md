...
| 2026-04-20 | Claude Code harness fallback (hybrid cloud + local) | claude-code-router (28k stars) preserves skills/hooks/MCP while swapping backend; DeepSeek API via router is honest quality-preserving path (local on 10GB can't match Opus); Ollama v0.14 ships native Anthropic shim; LiteLLM breaks Advisor Strategy; PAI-Pi = migration not fallback | `ai-infra/2026-04-20_claude-code-local-fallback.md` |
