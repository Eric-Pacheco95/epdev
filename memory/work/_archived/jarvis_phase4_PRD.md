...
> **Status:** active (Phase 4A complete; 4B next)  
...
Each line is **concise**, state-based, binary-testable.
...
Phases 4B-4D produce autonomous signals at 10-50x the rate of human sessions. Without a data layer, unbounded growth breaks downstream consumers (FTS indexer, heartbeat collectors, brain-map parser — all read processed/ via absolute paths or recursive globs).

**Architecture (validated by /first-principles, /find-logical-fallacies, /quality-gate 2026-03-28):**

- **Do NOT move files from processed/** — 6 consumers depend on the path. FTS indexer stores absolute paths; moving creates ghost entries.
- **Do NOT use LSM-tree compaction analogy** — synthesis aggregates (reads + summarizes), it does not merge-and-discard. Processed signals remain queryable.
- **DO use threshold-triggered retention** — delete consumed signals only after FTS index is verified to contain their content. Synthesis docs retain filename references.
- **DO build lineage index** — `signal_lineage.jsonl` for reverse-lookup (signal -> synthesis) without a separate manifest system.
- **DO schedule event rotation** — `rotate_events.py` already built; wire to monthly Task Scheduler.
- **DO monitor autonomous signal volume** — cap + alert on `Source: autonomous` signals to prevent runaway loops.
...
- [ ] Event rotation runs on monthly schedule | Verify: `schtasks /query /tn "\Jarvis\JarvisEventRotation"` returns Ready [E] [M]
- [ ] FTS index validated as canonical search layer | Verify: `jarvis_index.py search` returns content for a signal that was deleted from processed/ [E] [M]
- [ ] Autonomous signal rate collector fires alerts | Verify: heartbeat snapshot contains `autonomous_signal_rate` metric [E] [M]
- [ ] Reporting data contract documented for /vitals | Verify: `memory/work/jarvis/reporting_contract.md` exists with field definitions [E] [A]
- [ ] Processed signal retention activates at threshold | Verify: processed/ count stays below 2,000 after retention runs [I] [M]
...
- ~~ntfy integration~~ — retired 2026-03-27; all notifications routed via Slack.
